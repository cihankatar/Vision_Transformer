...
all_class_token = torch.zeros((n_images,1,token_dim))
print(class_token.shape)

for i in range (n_images):
    all_class_token[i,:,:] = class_token

print(all_class_token.shape)
...
n,number_tokens,token_size = tokens.shape
result = torch.zeros(n,number_tokens,token_size)
...
n,number_tokens,token_size = tokens.shape
result = torch.zeros(n,number_tokens*n_heads,token_size)

for idx,token in enumerate(tokens):   # 128 batch. each of 50x8, token size : 50x8   --> 50x8

    concat      = torch.zeros(n_heads,number_tokens,token_size)
...
linear_map = nn.Linear(100,50)
out=torch.zeros((96,50,8))
